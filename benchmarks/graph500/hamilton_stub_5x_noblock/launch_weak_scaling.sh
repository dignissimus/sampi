...
CONF_DIR="./benchmarks/graph500/hamilton_stub_5x_noblock/config"
SUITE_TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
export RESULTS_BASE_DIR="./benchmarks/graph500/hamilton_stub_5x_noblock/results/suite_${SUITE_TIMESTAMP}"
...
for conf_file in "${CONFIGS[@]}"; do
    ...
    if [ -f "$full_path" ]; then
        ...
        bash ./benchmarks/graph500/hamilton_stub_5x_noblock/submit_experiment.sh "$full_path"
