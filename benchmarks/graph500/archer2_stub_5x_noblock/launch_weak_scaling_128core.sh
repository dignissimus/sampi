...
CONF_DIR="./benchmarks/graph500/archer2_stub_5x_noblock/config/128-core-experiments"
SUITE_TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
export RESULTS_BASE_DIR="./benchmarks/graph500/archer2_stub_5x_noblock/results/128-core/suite_${SUITE_TIMESTAMP}"
...
for conf_file in "${CONFIGS[@]}"; do
    ...
    if [ -f "$full_path" ]; then
        ...
        bash ./benchmarks/graph500/archer2_stub_5x_noblock/submit_experiment.sh "$full_path"
