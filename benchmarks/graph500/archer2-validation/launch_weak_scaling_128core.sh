#!/bin/bash

CONF_DIR="./benchmarks/graph500/experiments/128-core-experiments"
export RESULTS_BASE_DIR="./benchmarks/graph500/experiments/results_archer2-validation_128core"

CONFIGS=(
    "1node_scale21.conf"
    "2node_scale22.conf"
    "4node_scale23.conf"
    "8node_scale24.conf"
    "16node_scale25.conf"
    "32node_scale26.conf"
)

echo "==========================================================="
echo " INITATING SAMPI WEAK SCALING BATTERY (128-CORE)"
echo "==========================================================="

for conf_file in "${CONFIGS[@]}"; do
    full_path="${CONF_DIR}/${conf_file}"
    
    if [ -f "$full_path" ]; then
        echo "--> Submitting: $conf_file"
        
        bash ./benchmarks/graph500/archer2-validation/submit_experiment.sh "$full_path"
        
    else
        echo "--> WARNING: $conf_file not found! Skipping..."
    fi
done

echo "==========================================================="
echo " Battery submitted successfully!"
echo " Run 'squeue -u \$USER' to monitor your queue status."
echo "==========================================================="
